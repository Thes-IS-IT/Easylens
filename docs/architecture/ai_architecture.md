...
EasyLens implements a hybrid on-device and cloud AI assistant system (Buddy) designed to act as an autonomous companion. It blends on-device local models, cloud fallbacks, semantic memory (Retrieval-Augmented Generation), and an autopilot router that executes app actions based on conversational intents.
...
- **Execution**: Runs on-device on the GPU/CPU using Google's AI Edge SDK.
- **Usage**: Primary conversational model for English speech input when offline.
- **Performance**:
  - Capped maximum generated output length to `maxTokens: 150` which reduces token compilation overhead.
  - Types replies character-by-character with a typewriter animation step (typing 3 characters every 15ms) to provide a responsive typing effect.
...
- **Usage**: Primary engine for Filipino/Tagalog queries, and acting as a fallback when the local model file is missing or when the user has active internet access.
- **Localization**: When the user's language is set to Tagalog, Gemini 3.6 Flash (Low) synthesizes natural Tagalog prompts based on Tagalog RAG contexts.
...
- **Usage**: Used during local development and testing when an external Ollama daemon is running on the host machine.
- **Supported Model**: `gemma2:2b` (local Gemma model backend via Ollama).
- **Role**: Acts as a zero-installation alternative during developer testing to test RAG workflows using Gemma without manually loading the binary into mobile storage.

---

### 02 — RETRIEVAL-AUGMENTED GENERATION (RAG) PIPELINE
...
### 03 — AUTONOMOUS NAVIGATION AGENT (LLM ROUTER)

Buddy functions as an autopilot controller that navigates the user through the app based on speech intents.
...
3. **Autopilot Transition**: If a routing command is detected, the app:
   ...
   - Executes `onNavigate(matchedKey)` to swap tabs or push screens autonomously.
...
### 04 — COMPLETE AI INTERACTION & DECISION LOOP
