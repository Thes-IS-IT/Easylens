...
import 'models/hazard_simulations.dart';

// HudMode enum moved to models/hud_mode.dart
...
class _HardwareScreenState extends State<HardwareScreen> with WidgetsBindingObserver {
  ...
  // Dynamic Hazard simulation lists — data extracted to models/hazard_simulations.dart
  final List<Map<String, dynamic>> _hazardSimulations = kHazardSimulations;
  ...
  void _onCameraFrameReceived(CameraImage image) {
    ...
    // Stop ALL frame processing & TTS when user navigates away from this screen
    ...
    if (_selectedHudMode == HudMode.objectDetection) {
      ...
      _isolateRunner.runInferenceRgb(rgbInput).then((results) {
        ...
        // Guard: user may have navigated away while isolate was running
  ...
  Future<void> _onEsp32FrameAvailable() async {
    if (!mounted) return;
    // Stop processing & TTS when user navigates away from this screen
    if (!(ModalRoute.of(context)?.isCurrent ?? false)) return;
  ...
  Future<Uint8List?> _captureCurrentImageBytes() async {
    ...
    if (_latestNv21Bytes != null && _latestWidth > 0 && _latestHeight > 0) {
      try {
        final jpegBytes = await compute(nv21ToJpegInIsolate, {
...
// YuvData, convertYuvToNv21, and nv21ToJpegInIsolate moved to models/yuv_data.dart
